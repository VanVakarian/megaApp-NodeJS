...
func (p *WSRealtimePublisher) PublishDiaryEntryUpdated(userID int64, entry DiaryEntry, historyEntry HistoryEntry, excludeClientID string) {
	p.hub.BroadcastToUser(userID, map[string]any{"type": "DIARY_ENTRY_UPDATED", "payload": map[string]any{"id": entry.ID, "newFoodWeight": entry.FoodWeight, "newKcals": entry.Kcals, "newHistoryEntry": historyEntry, "version": entry.Version}}, excludeClientID)
}
